...
class WebAuthnService(
        ...
        return runCatching {
            ...
            val authenticationRequest =
                AuthenticationRequest(
                    /* credentialId = */ credentialId,
                    /* userHandle = */ request.response.userHandle?.let { base64UrlDecode(it) },
    ...
    private fun buildCredentialRecord(
        storedCredential: CredentialStorage.CredentialRecord
    ): CredentialRecordImpl {
        val attestedCredentialData =
            AttestedCredentialDataConverter(objectConverter)
                .convert(storedCredential.publicKey)
        val transports = storedCredential.transports?.map { AuthenticatorTransport.create(it) }?.toSet()
