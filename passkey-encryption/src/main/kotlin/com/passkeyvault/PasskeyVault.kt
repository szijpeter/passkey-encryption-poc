...
import com.passkeyvault.storage.SaltManager
 ...
 *
 * ```kotlin
 * val vault = PasskeyVault(context)
 *
 ...
 * ```
 */
class PasskeyVault(context: Context) {
    ...
    private val saltManager = SaltManager(context.applicationContext)
    private val prfAuthenticator = PrfAuthenticator()
     ...
     *
     * @param activity The activity context (needed for biometric prompt)
    ...
    suspend fun authenticateForEncryption(
        ...
        return try {
            Log.d(TAG, "Starting PRF authentication for saltId: $saltId")

            // Get or create salt for this context
            val salt = saltManager.getOrCreateSalt(saltId)
            ...
            val key = KeyDerivation.deriveKey(prfOutput, saltId)
            val keyHash = KeyDerivation.getKeyHash(key)

            Log.d(TAG, "Encryption session created (keyHash: $keyHash)")

            Result.success(EncryptionSession(keyHash, saltId, key))
        } catch (e: Exception) {
     ...
     * @return EncryptedBlob containing ciphertext, IV, and saltId
     */
    fun encrypt(session: EncryptionSession, plaintext: String): EncryptedBlob {
     ...
     */
    fun decryptToString(session: EncryptionSession, blob: EncryptedBlob): String {
    ...
    fun hasSalt(saltId: String = DEFAULT_SALT_ID): Boolean {
        return saltManager.hasSalt(saltId)
    }
